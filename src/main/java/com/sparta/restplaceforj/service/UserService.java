...
public class UserService {
  ...
  public UserSignUpResponseDto createUser(UserSignUpRequestDto userSignUpRequestDto) {
    ...
    if (userRepository.existsByEmailAndUserStatusIs(userSignUpRequestDto.getEmail(),
        UserStatus.ACTIVE)) {
      throw new CommonException(ErrorEnum.DUPLICATED_EMAIL);
    }

    if (userRepository.existsByNicknameAndUserStatusIs(userSignUpRequestDto.getNickname(),
        UserStatus.ACTIVE)) {
  ...
  public UserProfileResponseDto updateUserProfile(UserUpdateRequestDto userUpdateRequestDto,
    ...
    if (userRepository.existsByNicknameAndUserStatusIs(nickname, UserStatus.ACTIVE)) {
      throw new CommonException(ErrorEnum.DUPLICATED_NICKNAME);
    }

    // 비밀번호 변경 로직
    String newPwd = userUpdateRequestDto.getNewPassword();
    String currentPwd = userUpdateRequestDto.getCurrentPassword();
    String confirmedPwd = userUpdateRequestDto.getConfirmPassword();
    if (!currentPwd.isBlank() && !newPwd.isBlank() && !confirmedPwd.isBlank()) {

      // 현재 비밀번호가 올바른지 확인
      if (!passwordEncoder.matches(currentPwd, password)) {
        throw new CommonException(ErrorEnum.BAD_PASSWORD);
      }
